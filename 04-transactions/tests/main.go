...
type eventPublisher interface {
	PublishPointsUsedAsDiscount(ctx context.Context, userID int, points int) error
}
...
func (p *TransactionProvider) Transact(ctx context.Context, txFunc func(adapters TransactableAdapters) error) error {
	tx, err := p.db.Begin()
	if err != nil {
		return err
	}
	...
	publisher, err := sql2.NewPublisher(
		tx,
		sql2.PublisherConfig{
			SchemaAdapter:        sql2.DefaultPostgreSQLSchema{},
	...
	adapters := TransactableAdapters{
		...
		EventPublisher: NewEventPublisher(publisher),
	}
...
func (r *UserRepository) Update(ctx context.Context, userID int, updateFn func(user *User) (bool, error)) (err error) {
	row := r.db.QueryRowContext(ctx, "SELECT email, points FROM users WHERE id = $1", userID)
	if row.Err() != nil {
		return row.Err()
	}
	...
	row = r.db.QueryRowContext(ctx, "SELECT discount FROM carts WHERE user_id = $1", userID)
	if row.Err() != nil {
		return row.Err()
	}
	...
	_, err = r.db.ExecContext(ctx, "UPDATE users SET points = points - $1 WHERE id = $2", user.Points(), userID)
	if err != nil {
		return err
	}
	...
	//_, err = r.db.ExecContext(ctx, "UPDATE carts SET discount = discount + $1 WHERE user_id = $2", user.Cart().Discount(), userID)
	//return err
}
...
type PointsUsedAsDiscount struct {
	UserID int `json:"user_id"`
	Points int `json:"points"`
}
...
func NewEventPublisher(publisher message.Publisher) *EventPublisher {
	return &EventPublisher{
		publisher: publisher,
	}
}

func (p *EventPublisher) PublishPointsUsedAsDiscount(ctx context.Context, userID int, points int) error {
	event := PointsUsedAsDiscount{
		UserID: userID,
		Points: points,
	}
	...
	return p.publisher.Publish("points_used_as_discount", msg)
}
...
type txProvider interface {
	Transact(ctx context.Context, txFunc func(adapters TransactableAdapters) error) error
}
...
func (h UsePointsAsDiscountHandler) Handle(ctx context.Context, cmd UsePointsAsDiscount) error {
	return h.txProvider.Transact(ctx, func(adapters TransactableAdapters) error {
		err := adapters.UserRepository.Update(ctx, cmd.UserID, func(user *User) (bool, error) {
		...
		err = adapters.EventPublisher.PublishPointsUsedAsDiscount(ctx, cmd.UserID, cmd.Points)
		if err != nil {
			return err
		}
